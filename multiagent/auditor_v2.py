"""
auditor_v2.py — QA Auditor CLI
================================
True service-parallel pipeline: each service runs scrape→audit→screenshot→report
independently. No stage cross-contamination between services.

Usage:
  python3 auditor_v2.py --html ./page.html --key AIza... [options]
  python3 auditor_v2.py --psid 1635 1837 --key AIza...
  python3 auditor_v2.py --esid 230 456 --key AIza...
  python3 auditor_v2.py --psid-file reports/services.csv --key AIza...

LangSmith tracing (optional):
  export LANGCHAIN_TRACING_V2=true
  export LANGCHAIN_API_KEY=ls__...
  export LANGCHAIN_PROJECT=bahrain-qa-auditor
"""
...
def main():
    ...
    if getattr(args, "resume", False):
        ...
        print(f"  [resume] Checkpoint found — {len(done)} services already completed")
        if not any([args.html, args.psid, args.esid, args.psid_file]):
            ...
            if not pending_psids:
                print("  [resume] All services in checkpoint are already done.")
                sys.exit(0)
            jobs = []
