...
from qa_agent import _detect_present_sections, _build_tailored_prompt
    ...
    with _checkpoint_lock:
        ...
        if os.path.exists(CHECKPOINT_FILE):
            try:
                ...
                on_disk = {}
        on_disk.update(checkpoint)
        on_disk[psid] = value
        checkpoint[psid] = value
...
def _run_service(job: dict, cfg: dict) -> dict:
    ...
    try:
        ...
        if cached:
            ...
            # Tailored prompt — only include checks relevant to present sections
            active_checks = _detect_present_sections(en_data, ar_data)
            ai_prompt = _build_tailored_prompt(
                en_data, ar_data, is_eservice, active_checks
            )
            ...
            validated_ai = R.validate_and_clean(raw_ai_issues, entity, service)
            all_issues = rule_issues + validated_ai

            audit_cache.set(psid, en_html, ar_html, all_issues)
            _log(
                f"  [{psid}] rules={len(rule_issues)} "
...
def run_pipeline(initial_state: dict) -> dict:
    ...
    # Shared config dict passed to every worker (read-only after construction)
    worker_cfg = {
        "gemini_key": gemini_key,
        "groq_key": groq_key,
        "openrouter_key": openrouter_key,
